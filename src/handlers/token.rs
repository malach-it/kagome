...
pub use super::responses::{
    AuthorizationCodeResponse, ClientCredentialsResponse, CodeChainResponse, GrantTypeResponse,
};
...
fn handle_validated_grant_type(
    ...
    match grant_type {
        GrantType::AuthorizationCode => {
            authorization_code(AuthorizationCodeResponse::from(token_response), request)
        }
...
fn authorization_code(
    token_response: AuthorizationCodeResponse,
    request: &KagomeRequest,
) -> Result<String, OAuthError> {
...
fn code_chain(
    ...
    client_id::validate(token_response, request)
        ...
        .and_then(|token_response| authorization_code::validate_optional(token_response, request))
